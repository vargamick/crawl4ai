...
class ProductSchema(BaseModel):
    ...
    # Fields the WooCommerce Store API supplies that HTML scraping never captured:
    sku: Optional[str] = Field(None, description="Product SKU / code(s)")
    price: Optional[str] = Field(None, description="Displayed price (may be a range/html-stripped)")
    attributes: Dict[str, List[str]] = Field(
        default_factory=dict, description="Spec attributes, e.g. {'pH Level': ['13.0 - 14.0'], 'Sizes': ['5L','200L']}")
