#!/usr/bin/env python3
"""
Docker-Compatible Batch Processing Script for Agar Product URLs
Scrapes all products from complete_agar_product_urls_20251008_122301.json using the Enhanced Agar Scraper within Docker container.
...
class DockerBatchAgarScraper:
    """
    Docker-compatible batch processor for scraping all Agar products with enhanced output formatting.
    """
    
    def __init__(self, urls_file: str, batch_size: int = 10, output_dir: str = "/app/agar_batch_output"):
        """
        Initialize batch scraper for Docker environment.
        
        Args:
            urls_file: Path to the JSON file containing product URLs
            batch_size: Number of URLs to process per batch (smaller for Docker)
            output_dir: Directory for output files (in container)
        """
        ...
        print(f"🐳 Docker Batch Agar Scraper Initialized")
...
async def main():
    ...
    batch_scraper = DockerBatchAgarScraper(
        urls_file=urls_file,
        batch_size=10,  # Smaller batches for Docker stability
        output_dir="/app/agar_batch_output"
    )
    ...
    print(f"📊 Check the output directory for results: /app/agar_batch_output/")

if __name__ == "__main__":
    print("🐳 Docker Agar Product Batch Scraper")
